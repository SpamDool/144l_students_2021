...
A1: In the chunk above, we created a new phyloseq object (sub_ps) that filters out sequences that correspond with chloroplasts and mitochondria in our original phyloseq object ps. We did this because we are only interested in the amplified bacterial sequences, not mitochondria and chloroplasts. These organisms are not part of our study.
...
A2: "Sequencing depth" means how many read counts were made in each sample sample during a read. It is how many total sequences were counted in each sample.
...
A3: The subsampling we did here will not have a large effect on our beta diversity because the mean of our read depth is similar before and after subsampling (7695 vs 6047).
...
A4: The two NMDS plots were made using subsampled (standardized) and non-subsampled datasets of NMDS ordinations of our full community samples. As both showed very similar patterns, we will use the standardized dataset because it does not seem to change diversity patterns.
...
A5: In both the San Diego and Santa Barbara data, the graphs showed more similarity between control and ash leachate at Day 0 than at the final timepoint, where the both the control and ash leachate community were relatively similar to other data points within the same community but more dissimilar to data of the other community, especially within the San Diego Data (the four data points at the final time point were more similar in the Santa Barbara data but seemed to show the same trend to a lesser degree). Also, the ash leachate community was more dissimilar to the initial timepoint community than the control community was, which was again more evident in the San Diego data.
...
A6: At timepoint 0, the control samples showed a significantly greater richness than the ash leachate, though the shannon index (total diversity) was not significantly different between control and ash leachate. At timepoint 6, both richness and diversity were significanly greater in the control than the ash leachate. Overall, this indicates that the ash leachate samples were less rich and diverse than the control (fewer ASVs and fewer dominant ASVs) by timepoint 6.
...
A7: Alteromonadales uniquely increased in relative abundance in ash leachate but not the control. Oceanospirillales and Campylobacterales showed the same trend but to a lesser degree. SAR 11 decreased in both, but far more so in the ash leachate than the control.
